...
7. `adjustPrices` lets sellers/buyers react locally to this tick's fill outcome: unsold asks decay toward marginal cost, sold-out asks rise; unfilled bids escalate, clamped to the wallet-grounded cap `max(MinUnitPrice, Cash/Hunger)` (see `docs/superpowers/specs/2026-07-16-wallet-grounded-bids-design.md`) so every posted price is backed by real money, with the `MinUnitPrice` floor keeping a transiently-insolvent factory's bid recoverable instead of stuck negative. Demand cascades backward through recipe tiers this way, one price signal at a time, with no global graph traversal.
